...
export function getMidPoint(one, another) {
  return {
    x: (one.x + another.x) / 2,
    y: (one.y + another.y) / 2,
  };
...
export function typeToCompositeOperation(type) {
  const operations = {
    pencil: 'source-over',
...
export function getMainCanvasTrimInfo(canvas, mainCanvas) {
...
// パスを描画
export function drawPathData(ctx, pathData) {
...
export function drawPaths(paths, targetCtx, tempCtx, scale = 1) {
...
export function resizeImage(dCanvas, dw, dh, sCanvas, sx, sy, sw, sh, tempCanvas = null) {
